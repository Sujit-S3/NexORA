...
class RecommendationService {
  ...
  static async getConciergeRecommendations(queryText, intentMemory = {}, userId, sessionId) {
    ...
    for (const p of relevantProducts) {
      ...
      // Fit Intelligence Injection
      if (pref && p.variants && p.variants.length > 0) {
        const fitRec = await FitIntelligenceService.getFitRecommendation(p, pref);
        if (fitRec) {
          p.fitRecommendation = fitRec;
          // Apply strict filtering: if AI knows preferred size, and the recommended size is NOT in stock, kill it.
          // Or if confidence is very low, maybe drop the matchScore.
          if (fitRec.confidence >= 50) {
            p.matchScore += 20; // boost score if we have a strong fit recommendation
          }
        }
      }
    }
    ...
    // Apply strict filtering (Never recommend Large only if preference is Medium)
    // We only keep products where the user's preferred size is in stock, IF they have one.
    const filteredProducts = relevantProducts.filter(p => {
      if (p.fitRecommendation && p.fitRecommendation.recommendedSize) {
        return true; // We found a recommended size in stock
      }
      // If we don't have a fit recommendation, but the product HAS sizes, and the user HAS a preference for this category, filter it out.
      if (pref && pref.preferredSizes && p.variants && p.variants.length > 0) {
        const categoryText = (p.category?.name || p.category?.slug || '').toLowerCase();
        const sizeType = FitIntelligenceService._determineSizeType(categoryText);
        const prefSize = pref.preferredSizes[sizeType];
        if (prefSize) {
          const inStock = p.variants.some(v => v.size.toLowerCase() === prefSize.toLowerCase() && v.stock > 0);
          if (!inStock) {return false;} // STRICT FILTER: out of stock in preferred size
        }
      }
      return true;
    });
